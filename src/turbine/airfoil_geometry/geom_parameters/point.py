...
class Point:
    ...
    def polynomial(self, second_point: "Point") -> poly.Polynomial:
        d = (
            (np.tan(th.rad(self.b)) + np.tan(th.rad(second_point.b)))
            / ((second_point.x - self.x) ** 2)
        ) - ((2 * (self.y - second_point.y)) / (self.x - second_point.x) ** 3)
        c = (
            ((self.y - second_point.y) / (self.x - second_point.x) ** 2)
            - (np.tan(th.rad(second_point.b)) / (self.x - second_point.x))
            - (d * (self.x + 2 * second_point.x))
        )
        b = (
            np.tan(th.rad(second_point.b))
            - 2 * c * second_point.x
            - 3 * d * second_point.x**2
        )
        a = (
            second_point.y
            - b * second_point.x
    ...
    def circle(self, second_point: "Point") -> cir.Circle:
        x_nominator = (
            (self.y - second_point.y)
            * np.tan(th.rad(self.b))
            * np.tan(th.rad(second_point.b))
            + self.x * np.tan(th.rad(second_point.b))
            - second_point.x * np.tan(th.rad(self.b))
        )
