...
class GeometryParameters:
    def __init__(
        ...
        ugt: float = 0,
        beta_in: float = 0,
        half_wedge_in: float = 0,
        Rle: float = 0,
        beta_out: float = 0,
        Rte: float = 0,
        Nb: int = 0,
        throat: float = 0,
        half_wedge_out: float = 0,
    ) -> None:
        self.R: float = R
    ...
    def def_values(self):
        ...
        if self.Rte >= 2:
            self.Rte = (
                ...
                * np.pi
                * (self.R / self.Nb)
                * np.cos(self.beta_out * np.pi / 180)
                / 2
            )
        if self.chord_x <= 0:
            self.chord_x = (
                ...
                * self.R
                / self.Nb
                * np.sin((self.beta_in - self.beta_out) * np.pi / 180)
            )
        if self.throat <= 0:
            self.throat = (
                2 * np.pi * self.R / self.Nb * np.cos(self.beta_out * np.pi / 180)
                - 2 * self.Rte
            )
        if self.ugt <= 0:
            self.ugt = 0.0001
        ...
        if self.chord_t < 20:
            ...
            self.chord_t = self.chord_x * np.tan(
                self.chord_t * np.pi / 180
            )  # chord_t = stagger angle
    ...
    def remove_throat_discontinuity(self, count=[0]) -> "td.RemoveThroatDiscontinuity":
        ...
        if np.abs(point2.y - yy2) < 0.00001:
            ...
            return td.RemoveThroatDiscontinuity(
                pressure_surf,
                suction_surf,
                point0,
                point1,
        ...
        print(
            "THE EXIT WEDGE ANGLE ITERATION FAILED. THE EXIT WEDGE ANGLE WANTS TO GO NEGATIVE. REDUCE THE EXIT BLADE ANGLE OR DECREASE THE THROAT."
        )
        print(
            f"Remove throat discontinuity was iterated {self.remove_throat_discontinuity.__defaults__[0][0]} times."
        )
        sys.exit()
    ...
    def find_suction_surface_throat_point(self) -> p.Point:
        ...
        y2 = ((2 * np.pi * self.R) / self.Nb) - (self.throat + self.Rte) * np.cos(
            th.rad(b2)
        )
    ...
    def find_suction_surface_leading_edge_tangency_point(self) -> p.Point:
        ...
        if self.chord_t == 0:
            self.chord_t = (
                ...
                + 180
                / np.pi
                * ((point2.x - x3) / (point2.b - b3))
                * np.log(np.cos(th.rad(point2.b)) / np.cos(th.rad(b3)))
                - self.Rle * np.cos(th.rad(b3))
            )
    ...
    def find_points_six_seven_eight_nine(self) -> pnb.PointNoBeta:
        ...
        return (
            pnb.PointNoBeta(x6, y6),
            pnb.PointNoBeta(x7, y7),
            pnb.PointNoBeta(x8, y8),
            pnb.PointNoBeta(x9, y9),
        )
