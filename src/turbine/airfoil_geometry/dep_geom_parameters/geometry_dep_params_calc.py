...
class GeometryDependentParametersCalculation:
    ...
    def find_zweifel_coefficient(
        self,
    ) -> float:  # INCOMPRESSIBLE ZWEIFEL LOADING COEFFICIENT
        return (
            ((4 * np.pi * self.gp.R) / (self.gp.Nb * self.gp.chord_x))
            * np.sin(th.rad(self.gp.beta_in - self.gp.beta_out))
            * (np.cos(th.rad(self.gp.beta_out)) / np.cos(th.rad(self.gp.beta_in)))
        )
    ...
    def find_blockage_in(self) -> float:
        return (
            2 * self.gp.Rle / (self.find_pitch() * np.cos(th.rad(self.gp.beta_in)))
        ) * 100

    def find_blockage_out(self) -> float:
        return (
            2 * self.gp.Rte / (self.find_pitch() * np.cos(th.rad(self.gp.beta_out)))
        ) * 100
    ...
    def find_lift_coefficient(self) -> float:
        return (
            ((self.find_pitch()) / self.find_chord())
            * (np.cos(th.rad(self.gp.beta_in)) + np.cos(th.rad(self.gp.beta_out)))
            * (np.tan(th.rad(self.gp.beta_in)) - np.tan(th.rad(self.gp.beta_out)))
        )
    ...
    def find_geometry_dependent_parameters(self) -> gdp.GeometryDependentParameters:
        ...
        return gdp.GeometryDependentParameters(
            pitch,
            stagger_angle,
            chord,
            zweifel_coefficient,
