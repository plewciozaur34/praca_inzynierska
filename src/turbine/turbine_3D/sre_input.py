...
class SimRadEquiInput:
    def __init__(
        ...
        self.phi_mean: float = (
            phi_mean  # współczynnik przepływu dla średniego promienia
        )
    ...
    def simple_radial_equi(self, rp_list, turbine_assum) -> pd.DataFrame:
        ...
        for r_p in rp_list:
            z1 = 1 + ((n + 1) / n) * (((1 - self.Rn_mean) / self.phi_mean) ** 2) * (
                (1 - r_p ** (2 * n))
                + (n / (n - 1))
                * (self.psi_mean / (1 - self.Rn_mean))
                * (1 - r_p ** (n - 1))
            )
            z2 = 1 + ((n + 1) / n) * (((1 - self.Rn_mean) / self.phi_mean) ** 2) * (
                (1 - r_p ** (2 * n))
                - (n / (n - 1))
                * (self.psi_mean / (1 - self.Rn_mean))
                * (1 - r_p ** (n - 1))
